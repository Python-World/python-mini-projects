...
@todo.command()
@click.pass_context
@click.option('-fin', '--fin_task_id', prompt='Enter ID of task to finish', type=int)
def done(ctx, fin_task_id):
    '''Delete a task by ID'''
    #Find task with associated ID
    if str(fin_task_id) in ctx.obj['TASKS'].keys():
        task = ctx.obj['TASKS'][str(fin_task_id)]
        #Delete task from task list in context
        del ctx.obj['TASKS'][str(fin_task_id)]
        click.echo('Finished and removed task "' + task + '" with id ' + str(fin_task_id))
        #Open todo.txt and write current index and tasks with IDs (separated by " ``` ")
        if ctx.obj['TASKS']:
            curr_ind = [str(ctx.obj['LATEST'] + 1)] 
            tasks = [str(i) + '```' + t for (i, t) in ctx.obj['TASKS'].items()]
        ...
        click.echo('Error: no task with id ' + str(fin_task_id))
